...
class HotelTest {
    @Test
    fun `should deserialize json object`() {
        val json =
            """
            [
              {
        ...
        assertEquals("Singapore", first.location.country)
        assertEquals(4, first.amenities.general.size)
        assertEquals(5, first.amenities.room.size)
        assertEquals(2, first.images.rooms.size)
        assertEquals("Double room", first.images.rooms.first().caption)
